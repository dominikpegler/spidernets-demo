...
def create_model(
    ...
    # search model on TIMM
        ...
        is_vit_like = run_config["model_name"].lower().startswith(("vit", "deit")) or ("dinov2" in run_config["model_name"].lower())
    ...
    if run_config["final_activation"] == "sigmoid":
        if n_dim > 1:
            raise NotImplementedError("SigmoidOutputLayers supports only 1 output dimension.")
